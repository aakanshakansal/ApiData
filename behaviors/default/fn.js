...
class LightPawn extends PawnBehavior {
  setup() {
    ...
    // const checkAPIAndSpeak = async () => {
    //   try {
    //     const response = await fetch(
    //       "https://full-duplex-dynalic-api.vercel.app/btn-state"
    //     );
    //     const data = await response.json();
    //     console.log(data.btnState);

    //     if (data.btnState === 1) {
    //       speakObjectName("Object name or any text you want to speak");
    //       displayServerInfo(clickedObject);
    //     } else if (data.btnState === 0) {
    //       stopSpeaking();
    //       hideServerInfo();
    //     }
    //   } catch (error) {
    ...
    // // Call this function to check the API and trigger the speaking functionality
    // checkAPIAndSpeak();

    // // Optionally, set an interval to continuously check the API value
    // setInterval(checkAPIAndSpeak, 5000); // Checks every 5 seconds
    ...
    const checkAPIAndSpeak = async () => {
      try {
        ...
        if (data.btnState === 1) {
          speakObjectName("Object name or any text you want to speak");
          displayHVACInfo1("Object_01001"); // Pass the relevant object here
        } else if (data.btnState === 0) {
          stopSpeaking();
          hideHVACInfo1();
        }
    ...
    // Call this function to check the API and trigger the speaking and HVAC info functionality
    checkAPIAndSpeak();

    // Optionally, set an interval to continuously check the API value
    setInterval(checkAPIAndSpeak, 5000); // Checks every 5 seconds

    const displayHVACInfo1 = (object) => {
